...
public class ReadTemplateStructures {
    ...
    public TemplateDomain getTemplateDomain(String fileName) throws JAXBException {
            ...
            @SuppressWarnings("unchecked")
            JAXBElement<TemplateDomain> unmarshalledObject = ((JAXBElement<TemplateDomain>) unmarshaller.unmarshal(new ByteArrayInputStream(fileContent.getBytes(Charset.forName("UTF-8")))));//new ByteArrayInputStream(fileContent.getBytes(StandardCharsets.UTF_8))));             //
